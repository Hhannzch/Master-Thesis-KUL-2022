...
class Decoder(nn.Module): 
  ...
  def generate_rl(self, images, features, voc_len, candidate_range, beam_size, alpha, start_index, valueNetwork, device):
    start_word = torch.tensor([[start_index, 0.0]]).to(device)
    ...
    while list(start_word[0][:-1].size())[0] < self.max_length:
        ...
        for i, (s) in enumerate(start_word):
            ...
            top_probs, top_labels_index = torch.topk(final_result, beam_size)  # top_preds.size(): batch_size x beam_size, top_labs.size(): batch_size x beam_size
...
def calculateAllValues(images, current_id, labels, valueNetwork):
  ...
  for i in labels:
    add_id = torch.tensor(i).type(torch.IntTensor).unsqueeze(0).to(device)
  ...
  captions_tensor = torch.stack(captions_list, 0).type(torch.IntTensor).to(device)
